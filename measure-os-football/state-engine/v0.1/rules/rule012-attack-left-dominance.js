(function () {
  ...
  function resolveState(counts) {
    ...
    if (
      counterConceded >= 2
      || (counterConceded >= 1 && leftAdvance === 0)
    ) {
      return { label: "🔴 左優位崩壊", status: "red" };
    }
    if (counterConceded === 1) {
      return { label: "🟠 左優位不安定", status: "orange" };
    }
    if (
      counterConceded === 0
      && leftAdvance > centralAdvance
      && leftAdvance > rightAdvance
      && (cross >= 1 || shot >= 1)
    ) {
      return { label: "🟢 左優位維持", status: "green" };
    }
    if (
      leftAdvance === 0
      || leftAdvance === centralAdvance
      || (leftAdvance > centralAdvance && leftAdvance > rightAdvance && cross === 0 && shot === 0)
    ) {
      return { label: "🟡 左優位停滞", status: "yellow" };
    }
    return null;
  }
