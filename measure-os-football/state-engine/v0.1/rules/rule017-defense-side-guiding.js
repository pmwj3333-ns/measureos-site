(function () {
  ...
  // 「サイド誘導」という戦略が、
  // 相手の攻撃を中央ではなくサイドへ誘導し、
  // 危険な中央侵入を抑えながら守備を維持できているかを評価する。
  //
  // Rule002（ハイプレス）= 前線から奪う / Rule003（ローブロック）= 深く守る
  // Rule016（ミドルブロック）= 中間で中央が単独最多でない状態を維持
  // Rule017（サイド誘導）= サイドへ追いやり、中央単独最多・被クロス多発を抑える
  const RULE_ID = "rule017";
  const PLAN_CATEGORY_KEY = "defense";
  const PLAN_OPTION = "サイド誘導";
  ...
  function resolveState(counts) {
    ...
    const crossConceded = counts[EVENT_CROSS_CONCEDED];
    ...
    if (shotConceded >= 2) {
      return { label: "🔴 サイド誘導崩壊", status: "red" };
    }
    if (shotConceded === 1) {
      return { label: "🟠 サイド誘導不安定", status: "orange" };
    }
    if (
      shotConceded === 0
      ...
      return { label: "🟢 サイド誘導維持", status: "green" };
    }
    if (
      (ballWon === 0 && frontLine === 0)
      || crossConceded >= 2
    ) {
      return { label: "🟡 サイド誘導停滞", status: "yellow" };
    }
    return null;
  }
